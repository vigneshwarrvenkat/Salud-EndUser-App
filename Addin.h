//
//  Addin.h
//  Salud Juicery ios
//
//  Created by Salud Juice Team (Vigneshwarr)on 10/30/15.
//  Copyright (c) 2015 Salud Juice Team. All rights reserved.
//

#import <Foundation/Foundation.h>

@interface Addin : NSObject



//@property (strong, nonatomic) NSString* thumbnail;

@property (strong, nonatomic) NSNumber* addinID;
@property (strong, nonatomic) NSString* addinName;
//@property (strong, nonatomic) NSString* ingredients;
//@property (strong, nonatomic) NSString* photoURL;

@property (strong, nonatomic) NSNumber* addinPrice;        //$$
//@property (strong, nonatomic) NSNumber* quantity;    //lbs
//@property (strong, nonatomic) NSNumber* maxHeight;    //in
//@property (strong, nonatomic) NSNumber* cuddleFactor; //out of 5



@end



